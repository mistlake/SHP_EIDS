...
# this data is now four years old. feel free to check the website and get some newer data 
...
#Because Google sucks, you should skip this part. I will show you on the projector.
install.packages("ggmap")
library(ggmap)
register_google(key="AIzaSyC01hb-OnmtjHTfD2CMUIxffQysrimYd6g")
map_dat <- get_googlemap(center = c(lon = median(usgs$longitude),lat=median(usgs$latitude)),zoom=6,maptype = "terrain",color="bw")
map <- ggmap(map_dat)
map + geom_point( aes(longitude, latitude) , data = usgs) + theme_bw()

#Now plot magnitude over time
ggplot(usgs) +
  aes(x=Date,y=mag) +
  geom_point() + 
  theme_bw() +
...
# when the y values change this much it is often due to the scaling being wrong. lets do a log transform 
# on the y axis
