...
setwd("C:/Users/owenw/Google Drive/Teaching_TA/Data Science Course_SHP/Crime/NYC Crime/")
...
streets <- getbb("New York New York")%>%
  opq()%>%
...
ggplot() +
  ...
  coord_sf(ylim = c(40.55, 40.9),
           expand = FALSE) 
### some of the later plots need to be updated to use open street
### map. see if you can do this
...
map <- get_map(c(-73.92133,40.73366))


ggmap(get_map(location = c(lon = -73.92,lat = 40.733), maptype = "toner-background")) +
  stat_density_2d(aes(Longitude, Latitude, fill = ..level..), 
                  geom = "polygon",
                  alpha = 0.25,
                  data = crime) +
...
shape <- readOGR("C:/Users/owenw/Google Drive/Teaching_TA/Data Science Course_SHP/Crime/NYC Crime/EastHarlem/EastHarlem_poly.shp")
plot(x = shape)
...
map_shape <- ggmap(get_map(location = center, zoom = 10))
